...
self.writer = cv2.VideoWriter(args.save_path, cv2.VideoWriter_fourcc(*'H264'), 
self.video.get(cv2.CAP_PROP_FPS), 
(int(self.video.get(cv2.CAP_PROP_FRAME_WIDTH)), 
int(self.video.get(cv2.CAP_PROP_FRAME_HEIGHT)))) 
if self.args.video_path == 0: 
self.writer1 = cv2.VideoWriter('static/videos/input.mp4', 
...
if self.args.video_path == 0: 
self.writer1.write(dst_img) 
dst_points, dst_shape, dst_face = select_face(dst_img, choose=False) 
if dst_points is not None: 
dst_img = face_swap(self.src_face, dst_face, self.src_points, dst_points, dst_shape, 
dst_img, self.args, 68) 
...
parser.add_argument('--correct_color', default=False, action='store_true', help='Correct 
color') 
